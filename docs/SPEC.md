...
fixtures and 12 parser error fixtures. Command metadata covers declaration
...
missing `kez`, required header colons, malformed `paralel yap` expressions,
unknown command typos such as `yzdır 1`, non-trailing required parameters after
default values, and multiline dictionary key errors. Those error fixtures also
compare the reported line, expected-token
