...
fixtures and 44 parser error fixtures. Command metadata covers declaration
...
missing `kez`, missing expression operands, required header names/colons,
malformed external-function, anonymous-function, `yeni`, postfix, collection,
and `paralel yap` expressions, unknown command typos such as `yzdır 1`,
