...
ROOT_RE = re.compile(
    r"^orhun-(?P<artifact>compiler|runtime)-"
...
def extract_archive(archive: Path, destination: Path) -> None:
    ...
    if archive.name.endswith(".tar.gz"):
        ...
        return
    raise RuntimeError("Release archive must end with .zip or .tar.gz")


def inspect_root(extracted: Path) -> tuple[Path, str, str, str, Path]:
    ...
    require(match is not None, f"Unexpected release archive root: {root.name}")
    artifact = match.group("artifact")
    if artifact == "compiler":
        require(
        ...
        executable = root / (
            "orhun-derleyici.exe"
            if (root / "orhun-derleyici.exe").is_file()
            else "orhun-derleyici"
        )
        require(executable.is_file(), "Compiler archive is missing orhun-derleyici")
    else:
        require(
            (root / "StdLib" / "orhun" / "temel.oh").is_file(),
            "Runtime archive is missing StdLib/orhun/temel.oh",
        )
        require((root / "VERSION").is_file(), "Runtime archive is missing VERSION")
        require(
            (root / "VERSION").read_text(encoding="utf-8").strip()
            == match.group("version"),
            "Runtime archive VERSION does not match its archive root",
        )
        executable = root / (
            "orhun.exe" if (root / "orhun.exe").is_file() else "orhun"
        )
        require(executable.is_file(), "Runtime archive is missing orhun executable")
    return (
        root,
        artifact,
        match.group("version"),
...
def default_install_root(artifact: str, version: str) -> Path:
    if os.name == "nt":
        base = Path(os.environ.get("LOCALAPPDATA", Path.home() / "AppData" / "Local"))
        return base / "Orhun" / artifact / version
    return Path.home() / ".local" / "share" / "orhun" / artifact / version
...
def publish_tree(
    source: Path, destination: Path, artifact: str, force: bool
) -> None:
    require(
    ...
    if destination_present:
        ...
        compiler_install = (
            destination / "bootstrap-compiler.manifest.json"
        ).is_file()
        runtime_install = (
            (destination / "VERSION").is_file()
            and (destination / "StdLib" / "orhun" / "temel.oh").is_file()
        )
        matching_install = (
            compiler_install if artifact == "compiler" else runtime_install
        )
        require(
            destination.is_dir()
            and not destination.is_symlink()
            and matching_install,
...
def create_shim(
    executable: Path, command_name: str, bin_dir: Path, force: bool
) -> Path:
    bin_dir.mkdir(parents=True, exist_ok=True)
    if os.name == "nt":
        shim = bin_dir / f"{command_name}.cmd"
        require(force or not shim.exists(), f"Command shim already exists: {shim}")
...
def install(args: argparse.Namespace) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="orhun-release-extract-") as raw:
        extracted = Path(raw)
        extract_archive(archive, extracted)
        root, artifact, version, platform_name, source_executable = inspect_root(
            extracted
        )
        if args.artifact is not None:
            require(
                artifact == args.artifact,
                f"Archive contains {artifact}, expected {args.artifact}",
            )
        platform_matches = platform_name.split("-", 1)[0] == host_os_name()
        require(
            args.allow_cross_platform or platform_matches,
            f"Archive platform {platform_name} does not match host {host_os_name()}",
        )
        require(
            platform_matches or args.no_shim,
            "Cross-platform staging requires --no-shim",
        )
        install_root = (
            args.install_root.expanduser().resolve()
            if args.install_root is not None
            else default_install_root(artifact, version).resolve()
        )
        executable_name = source_executable.name
        publish_tree(root, install_root, artifact, args.force)
    ...
    if not args.no_shim:
        ...
        command_name = "orhun-derleyici" if artifact == "compiler" else "orhun"
        shim = create_shim(installed_executable, command_name, bin_dir, args.force)

    result = {
        "format": f"orhun-{artifact}-install-v1",
        "artifact": artifact,
        "version": version,
    ...
    if args.json:
        ...
        print(f"Orhun {artifact} {version} installed: {install_root}")
        if shim is not None:
...
def parse_args(artifact: str | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Verify and install an Orhun compiler or runtime release archive"
    )
    ...
    parser.add_argument("--install-root", type=Path, help="Destination release directory")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Allow staging a release archive for another operating system",
    )
...
def main(artifact: str | None = None) -> int:
    return install(parse_args(artifact))


def cli(artifact: str | None = None) -> int:
    try:
